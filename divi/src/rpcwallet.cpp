...
Value getnewaddress(const Array& params, bool fHelp)
{
	...
	CPubKey newKey;
	if (!pwalletMain->GetKeyFromPool(newKey)) throw JSONRPCError(RPC_WALLET_KEYPOOL_RAN_OUT, "Error: Keypool ran out, please call keypoolrefill first");
...
Value getaccountaddress(const Array& params, bool fHelp)
{
	...
	return GetAccountAddress(params[0].get_str()).ToString();
}
...
Value setaccount(const Array& params, bool fHelp)
{
	...
	if (!address.IsValid()) throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid DIVI address");
	if (!IsMine(*pwalletMain, address.Get())) throw JSONRPCError(RPC_MISC_ERROR, "setaccount can only be used with own address");
	if (pwalletMain->mapAddressBook.count(address.Get())) {				// Handle case where address is the 'unused current key' of another account
...
Value getbalance(const Array& params, bool fHelp)
{
	...
	if (params.size() > 2) if (params[2].get_bool()) filter = filter | ISMINE_WATCH_ONLY;

	if (params[0].get_str() == "*") {
		// Calculate total balance a different way from GetBalance()
		// (GetBalance() sums up all unspent TxOuts while this sums up all received, sent and fees)
		...
		for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it) {
			...
			list<COutputEntry> listReceived, listSent;
			wtx.GetAmounts(listReceived, listSent, allFee, strSentAccount, filter);
			if (wtx.GetDepthInMainChain() >= nMinDepth) BOOST_FOREACH(const COutputEntry& r, listReceived) nBalance += r.amount;
...
Value movecmd(const Array& params, bool fHelp)
{
	if (fHelp || params.size() < 3 || params.size() > 5)
		throw runtime_error(
			...
			"3. \"comment\"       (string, optional) An optional comment, stored in the wallet only.\n"
			"\nResult:\n"
	...
	CAccountingEntry debit = CAccountingEntry({ strFrom, -nAmount, nNow, strTo, strComment, pwalletMain->IncOrderPosNext(&walletdb) });
	walletdb.WriteAccountingEntry(debit);
	CAccountingEntry credit = CAccountingEntry({ strTo, nAmount, nNow, strFrom, strComment, pwalletMain->IncOrderPosNext(&walletdb) });
...
Value sendmany(const Array& params, bool fHelp)
{
	...
	CWalletTx wtx;
	wtx.strFromAccount = params[0].get_str();
	...
	BOOST_FOREACH(const Pair& s, sendTo) {
		...
		if (!address.IsValid()) throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid DIVI address: ") + s.name_);
		if (setAddress.count(address)) throw JSONRPCError(RPC_INVALID_PARAMETER, string("Invalid parameter, duplicated address: ") + s.name_);
	...
	EnsureWalletIsUnlocked();
	CAmount nBalance = GetAccountBalance(wtx.strFromAccount, nMinDepth, ISMINE_SPENDABLE);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
	...
	string strAccount;
	if (params.size() > 2) strAccount = params[2].get_str();
