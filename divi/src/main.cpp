            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_DSTX) {
                    if (mapObfuscationBroadcastTxes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mapObfuscationBroadcastTxes[inv.hash].tx << mapObfuscationBroadcastTxes[inv.hash].address << mapObfuscationBroadcastTxes[inv.hash].vchSig << mapObfuscationBroadcastTxes[inv.hash].sigTime;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        if (strCommand == "tx") {
            ...
            vRecv >> tx >> address >> vchSig >> sigTime;
            ...
            if (pmn != NULL) {
                ...
                if (!obfuScationSigner.VerifyMessage(pmn->address, strMessage, vchSig)) {
                    LogPrintf("dstx: Got bad masternode signature from %s \n", pmn->address);
