...
void CMasternodeMan::AskForMN(CNode* pnode, CBitcoinAddress& address)
{
	std::map<CBitcoinAddress, int64_t>::iterator i = mWeAskedForMasternodeListEntry.find(address);
...
CMasternode* CMasternodeMan::Find(const CBitcoinAddress& address)
{
	LOCK(cs);
...
//
CMasternode* CMasternodeMan::GetNextMasternodeInQueueForPaymentX(int nBlockHeight, bool fFilterSigTime, int& nCount)
{
	LOCK(cs);
...
CMasternode* CMasternodeMan::GetCurrentMasterNodeX(int mod, int64_t nBlockHeight, int minProtocol)
{
	int64_t score = 0;
...
int CMasternodeMan::GetMasternodeRankX(const CTxIn& vin, int64_t nBlockHeight, int minProtocol, bool fOnlyActive)
{
...
std::vector<pair<int, CMasternode> > CMasternodeMan::GetMasternodeRanksX(int64_t nBlockHeight, int minProtocol)
{
	...
	BOOST_FOREACH(CMasternode& mn, vMasternodes) {
		mn.Check();
		if (mn.protocolVersion < minProtocol) { mn.score = 0;  continue; }
		if (!mn.IsEnabled()) { mn.score = 0; continue; }
...
CMasternode* CMasternodeMan::GetMasternodeByRankX(int nRank, int64_t nBlockHeight, int minProtocol, bool fOnlyActive)
{
	std::vector<pair<int64_t, CTxIn> > vecMasternodeScores;
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
	...
	if (strCommand == "mnb") { //Masternode Broadcast
		...
		if (nDoS > 0) {
			...
			CMasternode* pmn = Find(mnp.address);
...
void CMasternodeMan::Remove(CBitcoinAddress address)
{
	LOCK(cs);
